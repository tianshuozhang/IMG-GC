...
def load_model_bilp():
    ...
    processor,model_bilp = get_processor_and_model(modelpath=modelpath, device_map=device_map)

def load_pipe():
    global pipe
    model_path = r"C:\Users\17612\Desktop\course-management\img2img"
    if hasattr(torch.cuda, 'empty_cache'):
        torch.cuda.empty_cache()
    pipe = StableDiffusionImg2ImgPipeline.from_pretrained(model_path, torch_dtype=torch.float16, revision="fp16",
                                                          use_auth_token=True,device_map='sequential')
    # pipe = StableDiffusionImg2ImgPipeline.from_pretrained(model_path, device=device, use_auth_token=True)
